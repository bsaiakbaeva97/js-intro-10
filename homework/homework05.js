...
// Reduce
const countV = (str) => str.split('').reduce((acc, curr) => 'aeiouAEIOU'.includes(curr) ? acc + 1 : acc, 0);
...
const countCon = (str) => {
    return str.trim().split('').reduce((acc, curr) => !('aeiouAEIOU'.includes(curr) || ' '.includes(curr)) ? acc + 1 : acc, 0);
};
